...
public class LoginActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mEmailView.setText(PreferencesUtils.getString(this, "email", ""));
        mPasswordView.setText(PreferencesUtils.getString(this, "password", ""));
    ...
    public class UserLoginTask extends AsyncTask<Void, Void, Boolean> {
        ...
        protected Boolean doInBackground(Void... params) {
            ...
            StringBuffer result = Http.Post(Config.AuthUrl, jsonObject.toString(), ret);
            if(ret.retcode == 200){
                try {
                    JSONObject redata = new JSONObject(new String(result));
                    String msg = redata.getString("errmsg");
                    if(msg.equals("ok")){
                        PreferencesUtils.putString(mContext, "email", mEmail);
                        PreferencesUtils.putString(mContext, "password", mPassword);
                        return true;
                    }
                    else if(msg.equals("errorEmail")) {
                        errorCode = 1;
                        return false;
                    }
                    else if(msg.equals("errorPassword")){
                        errorCode = 2;
                        return false;
                    }
                } catch (JSONException e) {
        ...
        protected void onPostExecute(final Boolean success) {
            ...
            if (success) {
                finish();
                Intent intent = new Intent();
                intent.setClass(LoginActivity.this, ReceiveNotice.class);
                startActivity(intent);
            } else {
                if(errorCode == 1) {
                    mEmailView.setError(getString(R.string.error_invalid_email));
